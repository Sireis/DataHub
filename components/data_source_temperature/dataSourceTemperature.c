...
static int8_t convertToTemperature(uint32_t millVoltage);
...
{    
    ...
    {   
        ...
        if (_outboundQueue != NULL)
        {
            ...
            ESP_LOGI(TAG, "Sampled temperature (%d)", milliVoltage);
...
static int8_t convertToTemperature(uint32_t milliVoltage)
{
    int64_t temperature = milliVoltage * (125-(-55)) / 3300 - 55;
    return temperature;
}
